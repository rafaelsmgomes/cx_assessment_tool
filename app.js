...
var config = {
    // document: `${__dirname}/PDF/cx-content.html`,
    document: "http://www.pdfreactor.com/product/samples/textbook/textbook.html",
    // document: "http://dev.assessment-tools.com/PDF/cx-content.html",
    addLinks: true,
    pixelsPerInch:71,
...
app.get('/pdfreactor', (req, res) => {
    async function doSomething() {
        try{
            ...
            console.log(result)            
            // console.log(pdffile);
            fs.writeFile('./bin_dev/cxresult.pdf', result.document, 'base64', function(err) {
                if (err) {
                    console.log(err)
                }
            });
    ...
    doSomething()
    .then(() => {
        // res.contentType('application/pdf').send(pdffile)
        // res.render(renderFile);
        fs.readFile('./bin_dev/cxresult.pdf', (err, data) => {
...
app.get('/htmlversion/:id', (req, res) => {
    ...
    conn.query(`SELECT ans_value, question_id, ans_section FROM answers WHERE user_id = ? ORDER BY question_id ASC;
               ...
               `, [loggedID, loggedID, loggedID], (err, results) => {
                // 1578127744350
        if (err) throw err; 
        ...
        res.render( 'cx-content', { data: pdfData } );
    })    
})
